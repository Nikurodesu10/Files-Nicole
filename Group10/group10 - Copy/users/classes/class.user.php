...
class User{
	...
	private $DB_USERNAME='group10';
	private $DB_PASSWORD='123456';
	private $DB_DATABASE='group10';
	private $conn;
	...
	public function new_user($email,$password,$lastname,$firstname, $nickname, $access){
		...
		$data = [
			[$lastname, $firstname, $email, $password, $NOW, $NOW, '1', $access, $nickname],
		];
		$stmt = $this->conn->prepare("INSERT INTO tbl_users (user_lastname, user_firstname, user_email, user_password, user_date_added, user_time_added, user_status, user_access, user_nickname) VALUES (?,?,?,?,?,?,?,?,?)");
		try {
	...
	public function update_user($lastname,$firstname, $nickname, $access, $id){
    	...
    	$q->execute(array(':user_firstname'=>$firstname, ':user_lastname'=>$lastname, ':user_nickname'=>$nickname, ':user_date_updated'=>$NOW, ':user_time_updated'=>$NOW, ':user_access'=>$access, ':user_id'=>$id));
    	return true;
	}
